...
//$path = "ec2-18-224-246-6.us-east-2.compute.amazonaws.com/files/";
...
else //check if device exists
{
	...
	if ($result->num_rows>0) //device found
	{
		...
		$sql="Select * from `files` where `file_name` = '$requested_file'";
		$result=$dblink->query($sql) or
			die("Something went wrong with $sql");
		...
		if ($result->num_rows>0)
		{
			...
			$output[]="Status: OK";
			$output[]="MSG: File successfully retrieved from database for Device ID: $did";
			...
			//$file = $path.$retrieved_file["file_name"];
