...
if(!isset($_POST["submit"])) {
	...
	<form method="post" action="">
		  ...
		  <div><button type="submit" name="submit-device" value="submit-device">Submit</button></div>;
	</form>';
}

if(isset($_POST["submit-device"]) && $_POST["submit-device"] == "submit-device") {

	$type = $_POST["device-type"];
	$manufacturer = $_POST["device-manufacturer"];
	$serial_number = $_POST["serial-number"];
	
	$sql = "insert into `devices` (`device_type`, `manufacturer`, `serial_number`) values";
	$sql.="('$type', '$manufacturer', '$serial_number')";
	$dblink->query($sql) or
